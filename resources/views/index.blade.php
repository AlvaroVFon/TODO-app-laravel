        ...
        <ul class="grid grid-cols-5">
          ...
          <li class="flex items-center justify-center">
            <form action={{ url('edit/'.  $task->id) }}>
              @csrf      
              <button class="border border-blue-100 text-blue-400 hover:border-blue-400 duration-300 w-16 p-2 rounded">
                Edit
              </button>
            </form>
          </li>
          <li class="flex justify-center items-center">
            <form method="POST" action="{{ url('task/'.$task->id) }}" >
              @csrf
              @method('DELETE')
              <button type="submit" class="border border-red-100 text-red-400 hover:border-red-400 duration-300 p-2 rounded">
                 Delete
              </button>
          </form>
          </li>
